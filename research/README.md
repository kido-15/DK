...
```
[   정상  ] msit                 12건 추출 (예: 인공지능 기본법 시행령 제정안...)
[  실패   ] kisdi                응답은 받았으나 항목을 하나도 뽑지 못함 (link_pattern 확인 필요)
             └ RSS 후보 발견: https://www.kisdi.re.kr/rss.xml
             └ sources.json 갱신: type=rss, url=https://www.kisdi.re.kr/rss.xml
```

`대체 RSS를 찾지 못함`으로 뜨는 소스는 수동 보정이 필요합니다 → 아래 "소스 추가·수정" 참고.
