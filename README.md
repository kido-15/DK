...
- 대상 게시판: `sites.json`에 URL만 등록하면 추가됩니다 (개수 제한 없음).
  기본으로 **KISDI 보도자료 / 공지사항 / 전문가 칼럼** 3개가 등록되어 있고, 실제 수집까지 확인했습니다.
...
> **링크가 "(링크 없음)"으로 나올 때**
> 국내 기관 게시판은 주소 대신 `href="#none" onclick="goView('115116','')"` 처럼 자바스크립트 함수를 쓰는 곳이 많습니다.
> 이때 `--probe`가 `goView({arg0}=115116) 형태의 함수 호출입니다` 라고 함수 이름과 인자를 알려줍니다.
> 목록에서 글 하나를 직접 클릭해 상세 페이지 주소를 확인한 뒤, 그 인자가 들어가는 자리를 `{arg0}`으로 바꿔
> `detail_url_template`에 적으면 됩니다. (예: `https://www.kisdi.re.kr/bbs/view.do?key=m2101113055776&bbsSn={arg0}`)

> **목록 레이아웃이 섞여 있을 때**
> 최신 1건만 큰 카드로, 나머지는 줄 목록으로 보여주는 게시판이 있습니다(KISDI 전문가 칼럼).
> 셀렉터는 콤마로 여러 개를 나열할 수 있으므로 두 구조를 한 줄에 적으면 됩니다.
> `"row_selector": "div.board_list > ul > div.column, div.board_list > ul > li"`
...
- **해외 IP 차단**: 일부 국내 기관은 해외 IP를 막습니다. 그래서 서울 리전 Lambda나 국내 PC에서 실행해야 합니다
  (①번 자동화와 같은 이유). 접속이 불안정한 기관(과기정통부·방통위 등)이 있어 기본 재시도 횟수를 4회로 두었습니다.
- **날짜 위치**: 등록일이 제목 링크 바깥(`<td class="date">`)에 있든 안쪽(`<a>...<li>등록일 2026.09.15</li></a>`)에
  있든 모두 읽습니다. 다만 제목 안을 뒤질 때는 "3/4분기" 같은 표현을 날짜로 오인하지 않도록 **연도가 있는 표기만** 받습니다.
